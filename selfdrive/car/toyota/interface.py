...
class CarInterface(CarInterfaceBase):
  ...
  def _get_params(ret, candidate, fingerprint, car_fw, experimental_long, docs):
    ...
    if (candidate in TSS2_CAR or ret.enableGasInterceptor) and Params().get_bool("TSS2Tune"):
      ...
      tune.kpBP = [0., 5., 20.]
      tune.kpV = [1.0, 1.0, 0.7]

      # In KMH  = [  0,   43,   72,  97,  144]
      tune.kiBP = [ 0.,  12.,  20., 27., 40.]
      tune.kiV =  [1., 1., .195, .10, .01]
      if candidate in TSS2_CAR:
        ret.vEgoStopping = 0.4         # car is near 0.1 to 0.2 when car starts requesting stopping accel
        ret.vEgoStarting = 0.4         # needs to be > or == vEgoStopping
