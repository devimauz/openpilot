...
class CarInterface(CarInterfaceBase):
  ...
  def _get_params(ret, candidate, fingerprint, car_fw, experimental_long, docs):
    ...
    if (candidate in TSS2_CAR or ret.enableGasInterceptor) and Params().get_bool("TSS2Tune"):
      ...
      tune.deadzoneBP = [16., 16., 20., 30.]
      tune.deadzoneV = [0.03, .03, .06, .15]
      tune.kpBP = [0., 5., 20.]
      tune.kpV = [3.0, 3.0, 0.7]

      # In MPH  = [  0,   27,   45,  60,  89] Frogtest
      tune.kiBP = [ 0.,  12.,  20., 27., 40.]
      tune.kiV =  [.05, .215, .195, .10, .01]
      if candidate in TSS2_CAR:
        ret.vEgoStopping = 1.0         # car is near 0.1 to 0.2 when car starts requesting stopping accel
        ret.vEgoStarting = 1.0         # needs to be > or == vEgoStopping
        ret.stopAccel = -2.5          # Toyota requests -0.4 when stopped
        ret.stoppingDecelRate = 1    # reach stopping target smoothly
